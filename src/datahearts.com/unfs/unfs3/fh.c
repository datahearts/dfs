 ...
 */
post_op_fh3 fh_comp_type(uint64 ino, const char *path, unsigned int type)
{
    post_op_fh3 result;
    go_statstruct buf;

    if (go_lstat(ino, path, &buf) != NFS3_OK || (buf.st_mode & type) != type) {
