 ...
 */
post_op_attr get_post(const uint64 ino, const char *path, struct svc_req * req)
{
    go_statstruct buf;
    if (go_lstat(ino, path, &buf) != NFS3_OK) {
        return error_attr;
    }
 ...
 * return pre-operation attributes
 */
pre_op_attr get_pre(uint64 ino)
{
    pre_op_attr result;
    go_statstruct buf;

    if (go_lstat(ino, "", &buf) != NFS3_OK) {
 ...
 */
mode_t get_mode(sattr3 new)
{
    if (new.mode.set_it == TRUE)
        return new.mode.set_mode3_u.mode;
    else
        return S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IXOTH;
}
